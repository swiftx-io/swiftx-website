        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className={cn(
                'flex items-start space-x-4 p-6 sm:p-8 rounded-lg border',
            ...
            >
              <CheckCircle className="w-6 h-6 text-primary dark:text-primary-dark flex-shrink-0 sm:w-7 sm:h-7" />
              <p className="text-base sm:text-lg lg:text-xl text-foreground dark:text-foreground-dark">{benefit}</p>
            </div>
          ))}
        </div>
      </div>
